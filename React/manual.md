...
**Action** - объекты, у которых свойство *type* указывает на метод изменения состояния и плюс дополнительные данные, в соответствии с которыми может изменяться состояние.   
...
**Reducers** - функции, которые получают текущее состояние, экшн и возвращают новое состояние. В соответствии с экшеном выбирается метод изменения состояния. По умолчанию необходимо, чтобы ред.серы возвращали старое состояние. 
```javascript
...
**Stor** - объект, который позволяет работать совсестно эекшнам и редюсерам, плюс содержит состояние, предоставляет доступ к состоянию, изменить его и зарегистрировать слушатели.

Порядок действия:  
1. Вызывается ```store.dispatch(action)```;  
2. Стор вызывает редюсер в соответствии с экшеном;  
3. Сохраняется состояние, вызываются зарегистированные слушатели.

Функции:  
**createStore(reducer, [preloadedState], [enhancer])** - создает стор, который содержит полностью единственное состояние приложения. Принимает редюсер, который возвращает новое дерево состояния, начальное состояние и расширитель стора(например, applyMiddleware()). Когда стор создан, Redux отправляет фиктивный экшен для редюсера, для заполнения стора с начальным состоянием. Возвращаемые объект содержит:  
* **getState** - возвращет текущее состояние приложения.  
* **dispatch(action)** - отправляет экшн. Редюсеры не могут отправлять экшены. Возвращает посланный экшн.  
* **subscribe(listener)** - добавляет слушателя. Из слушателя можно вызвать ```dispatch```. Принимает колбэк.  
* **replaceReducer(nextReducer)** - заменяет редюсер, который в настоящее время используется стором, чтобы вычислить состояние.  

## Async Actions <a name='async-actions'></a>
